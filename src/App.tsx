...
import { Lock, ShieldCheck, Info, MonitorSmartphone, X, ChevronDown } from 'lucide-react';
    ...
    <div className="min-h-screen bg-neutral-50 text-neutral-900 flex flex-col font-sans selection:bg-blue-100 selection:text-blue-900">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        <div className="space-y-4">
          <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 sm:p-5 shadow-sm transition-all duration-200">
            <button 
            ...
            >
              ...
              <div className="flex-shrink-0 ml-4 text-amber-500">
                <ChevronDown className={`w-5 h-5 transition-transform duration-200 ${isMobileExpanded ? 'rotate-180' : ''}`} />
              </div>
            </button>
            {isMobileExpanded && (
              <div className="mt-3 ml-10 text-xs sm:text-sm text-amber-800 leading-relaxed">
                因網頁腳本機制之限制，本系統<strong>目前不支援手機操作</strong>。為了確保您能順利進行加班明細申報，請務必使用<strong>電腦版網頁瀏覽器 (如 Chrome, Edge 等) </strong>來開啟並使用本系統。
              </div>
            )}
          </div>

          <div className="bg-blue-50 border border-blue-200 rounded-2xl p-4 sm:p-5 shadow-sm transition-all duration-200">
            <button 
            ...
            >
              ...
              <div className="flex-shrink-0 ml-4 text-blue-500">
                <ChevronDown className={`w-5 h-5 transition-transform duration-200 ${isSecurityExpanded ? 'rotate-180' : ''}`} />
              </div>
            </button>
            {isSecurityExpanded && (
              <div className="mt-4 ml-10 text-xs sm:text-sm text-blue-800 space-y-2 leading-relaxed border-t border-blue-200/50 pt-3">
                <p>
